...
class AttentionBiGRU(nn.Module):
    def __init__(self, input_shape, n_units, id_to_aa, dropout, d):
        ...
        self.embedding = nn.Embedding(len(id_to_aa) + 1, d, padding_idx=0)
        ...
        self.attention = AttentionWithContext(
            n_units
            * 2,  # biderectional so hidden from left to right and from right to left
...
class TimeDistributed(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.batch_first:
            part_att_vec_dr = part_att_vec_dr.contiguous().view(
                x.size(0), -1, part_att_vec_dr.size(-1)
            )
            aa_att_coeffs = aa_att_coeffs.contiguous().view(
                x.size(0), -1, aa_att_coeffs.size(-1)
            )
        else:
            part_att_vec_dr = part_att_vec_dr.view(
                -1, x.size(1), part_att_vec_dr.size(-1)
            )
...
class HAN(nn.Module):
    def __init__(self, input_shape, n_units, id_to_aa, d, dropout):
        ...
        self.attention = AttentionWithContext(n_units * 2, return_coefficients=True)
        self.lin_out = nn.Linear(n_units * 2, 18)
        self.preds = nn.Softmax()

    def forward(self, part_ints):
        part_att_vecs_dr, aa_att_coeffs = self.timeDistributed(
            part_ints
        )  # time distributed
