...
def main():
    ...
    if model_type == "LSTM":
        ...
        sequences_train = np.array(
            [
                np.pad(prot, (0, length_padding - len(prot)), constant_values=(0, 0))
                for prot in sequences_train
            ]
        )
        sequences_test = np.array(
            [
                np.pad(prot, (0, length_padding - len(prot)), constant_values=(0, 0))
                for prot in sequences_test
            ]
        )

        # let's create a train and a valid dataset
        X_train, X_valid, y_train, y_valid = train_test_split(
            sequences_train, torch.Tensor(y_train), test_size=0.2
        )
    ...
    if model_type == "HAN":
        ...
        sequences_train = np.array(
            [
                [
                    prot[i * (len(prot) // 3) : (i + 1) * (len(prot) // 3)]
                    for i in range(3)
                ]
                for prot in sequences_train
            ],
        ...
        sequences_train = np.array(
            [
                [
                    np.pad(
                        part, (0, length_padding - len(part)), constant_values=(0, 0)
                    )
                    for part in prot
                ]
                for prot in sequences_train
